...
compare.avg.aph <- compare.aph %>%  
  ...
  summarize_at(.vars = c("conANI", "popANI", "percent_genome_compared", "coverage_overlap"), .funs = mean)
...
plot_aph.abund <- relabund.aph_wide %>% ggplot(aes(x = sampledate, y = relabundance, color = category)) +
  geom_point() +
  geom_line() +
  facet_wrap(~year, ncol = 1, scales = "free_x") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 0, vjust = 0.5, hjust= 0.5, face = "bold", size = 12),
...
plot_maer.abund <- relabund.maer_wide %>% ggplot(aes(x = sampledate, y = relabundance, color = category)) +
  geom_point() +
  geom_line() +
  facet_wrap(~year, ncol = 1, scales = "free_x") +
  theme_classic() +
...
blooms.aph$sampledate
blooms.maer$sampledate
